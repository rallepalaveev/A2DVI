# A2DVI.v4.6: Digital Video card for Apple II Computers with multifunction capabilities

## Preface

This is a firmware project for a digital DVI/HDMI video card for Apple II. It directly produces a digital video stream from the Apple II's memory content. The signal is output via an HDMI connector, connecting the Apple II to modern displays with HDMI (or DVI) inputs. No analog signal conversion is required.

The project is a collaboration between Ralle Palaveev and Thorsten Brehm. 

The hardware is based on the PICO controller board. Both, original PICO (RP2040) and PICO2 (RP2350) modules are supported. Separate firmware is provided for the two PICO variants.
...
Only one firmware is active at a time. A firmware can be selected by an initial selection menu, which can be invoked within 15 seconds of powering the computer by calling PR#N command, where N is the slot number.

Once the computer is powered, DVI is always active during the 15s timeout, so that the user can see the menu for firmware selection. If the DVI firmware is not the one selected, the bottom of the screen displays which firmware would become active and the timeout counter. During the initial 15s timeout the LED blinks very fast.

Once a firmware is selected, the timeout ends immediately and the selected firmware becomes active.

Alternatively, if during the 15 second timeout nothing is done, upon timeout completion the previously selected firmware becomes active.
...
3. Apple II Mouse Card (PAL). This firmware turns the card into a Mouse controller - it has to be installed in Slot4 and a USB mouse connected to the USB-c port of the card (an adapter may be needed for USB-A mouse). Once the firmware becomes active (15s after power-on), the card can be used by programs like A2Desktop.

4. Apple II Mouse Card (NTSC). This firmware turns the card into a Mouse controller - it has to be installed in Slot4 and a USB mouse connected to the USB-c port of the card (an adapter may be needed for USB-A mouse). Once the firmware becomes active (15s after power-on), the card can be used by programs like A2Desktop.

5. When FDD is selected and activated, the LED blinks slowly until the 2 floppy disk images are selected; calling PR#N displays a menu to select disk images for the 2 floppy disk drives. The 140kB .dsk images must be on a thumb drive connected to the USB-c port of the card. After selecting the disk images, the LED blinks synchronously with the floppy disk access. Currently works on IIe only. If programs require to change disks: The current solution is to connect an external button (A2DVIplus has a two pin connector to connect a button/switch). The firmware detects floppy images with names only differing in a single digit. So, the trick is to prepare the disk images to follow a naming scheme. "Oregon Trail - Disk1.dsk", "Oregon Trail - Disk 2.dsk" etc. Then you just select the boot disk via the menu. And when the program later asks to flip/cycle disks, you simply press the button to advance one digit ahead. The firmware automatically cycles through the disk images - in ascending order. Just sometimes requires a bit of preparation. Something like "Prince of Persia (boot disk).dsk", "Prince of Persia (side a).dsk" doesn't work, so some images need to be renamed.
